...
| `app/Peripherals/IPeripheral.cs`, `app/Peripherals/Keyboard/**`, `app/Peripherals/Mouse/**` | `source/GHelper.Shared/Peripherals/**` | Moved (Milestone 2 Story 5, slice 5), 40 of the folder's 41 files, namespaces (`GHelper.Peripherals`, `.Keyboard`, `.Keyboard.Models`, `.Mouse`, `.Mouse.Models`) unchanged. One content edit: `AsusMouse`'s twelve default combo commands were spelled with `System.Windows.Forms.Keys`, which the shared layer cannot reference, so they now use a private `Vk` constant holder in the same file. The numeric values are identical — the twelve emitted command strings were compared against the WinForms `Keys` output and match byte for byte. |
| `app/Peripherals/PeripheralsProvider.cs` | `source/app/Peripherals/PeripheralsProvider.cs` | **Deliberately stayed** in the head. It is the only file in the folder that touches the head: `RefreshHotkeys` and `UpdateSettingsView` drive `Program.settingsForm` / `Program.inputDispatcher`, and both are called from its own connect/disconnect/battery flows rather than from the edges, so it cannot be shimmed the way `ProcessHelper` was. Moving it needs a callback seam the WPF head can also implement; that is a later slice. |
| `app/AnimeMatrix/Communication/Device.cs`, `Packet.cs`, `Platform/UsbProvider.cs`, `Platform/WindowsUsbProvider.cs` | `source/GHelper.Shared/AnimeMatrix/Communication/**` | Moved with slice 5, namespace `GHelper.AnimeMatrix.Communication(.Platform)` unchanged. Not tidiness — `AsusKeyboard` and `AsusMouse` both derive from `Device`, so the HID transport had to move with them. One content edit: `Packet`'s constructor went from `internal` to `protected`, because `AnimeMatrixPacket` and `SlashPacket` still derive from it from the head assembly. The rest of `app/AnimeMatrix/` (`AniMatrixControl.cs`, `AnimeMatrixDevice.cs`, `MatrixFont.cs`, `SlashDevice.cs`) has not moved. |
| `app/USB/Aura.cs` | `source/app/USB/Aura.cs` + `source/GHelper.Shared/USB/AuraMode.cs` + `source/GHelper.Shared/USB/AuraSpeed.cs` | Split, narrowly (slice 5). The `AuraMode` and `AuraSpeed` enums moved because the peripheral classes map laptop Aura state onto device lighting; they keep the `GHelper.USB` namespace, so every call site on both sides resolves unchanged — the same pattern used for `AsusFan` in slice 3. `AuraPower`, `AuraBacklightType` and the `Aura` class itself stayed: `Aura` uses `Properties.Strings`, `Program.acpi` and `InputDispatcher`. |
...
Top-level `source/app/` folders as they currently stand: `Ally/`, `AnimeMatrix/`, `AutoUpdate/`, `Battery/`, `Display/`, `Fan/`, `Gpu/`, `Helpers/`, `Input/`, `Mode/`, `Overlay/`, `Pawn/`, `Peripherals/`, `Properties/`, `Resources/`, `UI/`, `USB/` — all still present, though `Peripherals/` is now down to `PeripheralsProvider.cs` and `AnimeMatrix/Communication/` is gone entirely.

`GHelper.Shared.csproj` picked up two things with slice 5: a `HidSharpCore` package reference (the peripheral classes talk HID directly), and an explicit `<Using Include="System.Drawing" />`. The second is not a new dependency — `System.Drawing.Color` is in the base framework (`System.Drawing.Primitives`), and the head only ever resolved it because `UseWindowsForms` adds that implicit using. Declaring it at the project level keeps the ~50 `Color` references in the moved files free of `using` churn that would conflict on an upstream merge.
