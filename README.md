...
Early-stage fork. Milestone 1 (repository and context setup) is in progress; the WinForms application (`app/`) is still the only working executable head. See [Current Status](#current-status) below for what has and hasn't been verified yet.

## Getting Started

Open `app/GHelper.sln` in Visual Studio and build/run `app/GHelper.csproj` — this is currently the same WinForms application as upstream G-Helper. No separate WPF project or shared class library exists yet; both are introduced in Milestone 2.

## Current Status

- The fork's exact relationship to `upstream/seerge/g-helper` (a dedicated git remote, confirmed GitHub fork status) has not been verified from this environment.
- A clean-checkout build/launch pass on a ROG Ally has not yet been recorded as a baseline.
